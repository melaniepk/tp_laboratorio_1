...
Empleado PedirDatosEmpleado()
{
    Empleado miEmpleado;
		...
		utn_getPalabra(miEmpleado.nombre ,sizeof(miEmpleado.nombre), "\nIngrese el nombre del empleado", "\nError!. ", 10);
		utn_getPalabra(miEmpleado.apellido ,sizeof(miEmpleado.apellido ), "\nIngrese el apellido del empleado", "\nError!. ", 10);
...
int OrdenarEmpleados(Empleado *list, int len, int *orden)
{
	int retorno = -1;
	int i;
	int j;
			...
			for (i = 0; i < len - 1; i++)
			{
				for (j = i + 1; j < len; j++)
				{
					if (list[i].isEmpty == FALSE && list[j].isEmpty == FALSE)
					{
						...
						{
							if((list[j].sector < list[i].sector)==0 &&
									strcmp(list[i].apellido, list[j].apellido)							)
			...
			for (i = 0; i < len - 1; i++)
			{
				for (j = i + 1; j < len; j++)
				{
					if (list[i].isEmpty == FALSE && list[j].isEmpty == FALSE)
					{
						...
						{
							if((list[j].sector > list[i].sector)==0
									&& strcmp(list[j].apellido, list[i].apellido))
...
void MostrarTotalYPromedioDeSalarios(Empleado list[], int len)
{
	...
	if(list != NULL && len > 0)
	{
		...
		for(i=0;i<len;i++)
		{
			if(list[i].salario > promedio && list->isEmpty == FALSE)
			{
				mayorAPromedio++;
			}
		}
